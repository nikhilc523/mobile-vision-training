"""
BiLSTM Fine-Tuning for Deployment - Phase 2.5

Fine-tune the existing best BiLSTM model for 5 epochs with low learning rate
to polish weights, then evaluate multiple thresholds for deployment modes:
- Balanced mode (F1-optimal)
- Safety mode (Recall-optimal)
- Precision mode (Precision-optimal)

Target: F1 ≥ 0.78, ROC-AUC ≥ 0.93
"""
...
def find_deployment_thresholds(y_true, y_pred_proba, thresholds):
    ...
    Returns:
        Dictionary with optimal thresholds for each mode
    """
...
def main():
    parser = argparse.ArgumentParser(description='Phase 2.5 - BiLSTM Fine-Tuning for Deployment')
    ...
    print("="*70)
    print("PHASE 2.5 — BiLSTM FINE-TUNING FOR DEPLOYMENT")
    print("="*70)
    ...
    print(f"Learning rate: {args.lr} (low LR for polishing)")
    ...
    # Fine-tune (no early stopping - just 5 epochs)
    print("\n" + "="*70)
    ...
    checkpoint_path = Path('ml/training/checkpoints/lstm_bilstm_final_best.h5')
